<?php 
    ...
    $breadcrumb['link'] = "/symfony/web/index.php/admin/viewSystemUsers"; ?>

    <?php 
    if($currencyList > 0) {
        $breadcrumbnew['name']= "Edit User";
        $breadcrumbnew['link'] = null;
    } else {
        $breadcrumbnew['name']= "Add User";
        $breadcrumbnew['link'] = null;
    } ?>
...
<div id="systemUser" class="box">
    ...
    <div class="inner">
        ...
        <form name="frmSystemUser" id="frmSystemUser" method="post" action="" >
            ...
            <ul class="disc">
                  <li>
                        <?php echo  ' <span class="boldText"> If user role is a Functional Head, Assign atleast one subordinate to Functional Head.</span><br></br>'; ?>
                    </li>
...
$("#frmSystemUser").ready(function() {
 if ($('#systemUser_userType').val() == 8 ) {
    $("label[for='systemUser_FunctionalHeadDept']").parent('li').addClass('FunctionalHeadDept').show();
  } else {
    $("label[for='systemUser_FunctionalHeadDept']").parent('li').addClass('FunctionalHeadDept').hide();
  }
});
$("#frmSystemUser").change(function() {  
 if ($('#systemUser_userType').val() == 8) {
    $("label[for='systemUser_FunctionalHeadDept']").parent('li').addClass('FunctionalHeadDept').show();     
  } else {
     $("label[for='systemUser_FunctionalHeadDept']").parent('li').addClass('FunctionalHeadDept').hide(); 
