    ...
    public function execute($request) {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                if(($this->form->getValue('userType') == 8) &&  ($this->form->getValue('FunctionalHeadDept')=="") )
                {
                    $this->getUser()->setFlash('warning', __("Assign Atleast One Department(s) To Functional Head"));
                    $this->redirect('admin/saveSystemUser?userId='. $userId);
