...
class LocationDao extends BaseDao {
	...
	public function getNumberOfEmplyeesForLocation($locationId) {
		...
		// }
		try{
		$q="SELECT count(hs_loc.emp_number) AS count_empNumber 
			FROM `hs_hr_emp_locations` AS hs_loc 
			LEFT JOIN  
			`hs_hr_employee` AS hs_emp 
			ON 
			hs_emp.emp_number=hs_loc.emp_number 
			WHERE 
			hs_emp.termination_id IS NULL 
			AND 
			hs_loc.location_id=$locationId";
