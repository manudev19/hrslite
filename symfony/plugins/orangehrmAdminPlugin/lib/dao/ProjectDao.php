...
class ProjectDao extends BaseDao {
    ...
    public function deleteCostCenterMapping($id) {

         try {
            $q = Doctrine_Query::create()
                            ->delete('CostCenterMapping')
                            ->whereIn('id', $id);
	...
	public function getProjectById($projectId) {
        ...
        public function getEditCostCenterById($id){
        
        try {
            return Doctrine :: getTable('CostCenterMapping')->find($id);
        } catch (Exception $e) {
            throw new DaoException($e->getMessage());
        }
    }
    ...
    public function getCostCenterMappingListCount(){
        try{
            $q = "SELECT cost_center_mapping.id from cost_center_mapping
        LEFT JOIN cost_center ON  cost_center.id = cost_center_mapping.costcenter_id
        LEFT JOIN hs_hr_employee on hs_hr_employee.emp_number = cost_center_mapping.empId
        LEFT JOIN ohrm_location ON ohrm_location.id = cost_center_mapping.location";
        $pdo = Doctrine_Manager::connection()->getDbh();
            $query = $pdo->prepare($q);
    ...
    /*Returns the cost mapping */
	public function getCostCenterByAdmin(){

		   try {
            // GetCostCenterMapping
		$q= "SELECT cost_center_mapping.costcenter_id,  cost_center_mapping.location, cost_center_mapping.empId,hs_hr_employee.emp_firstname, concat(hs_hr_employee.emp_firstname, ' ', hs_hr_employee.emp_lastname) AS employeeFullName, cost_center.name, cost_center_mapping.id as id, ohrm_location.name AS LocationName  from cost_center_mapping
		LEFT JOIN cost_center ON  cost_center.id = cost_center_mapping.costcenter_id
		LEFT JOIN hs_hr_employee on hs_hr_employee.emp_number = cost_center_mapping.empId
        LEFT JOIN ohrm_location ON ohrm_location.id = cost_center_mapping.location";
     ...
     /*Returns the cost mapping */
    public function getSavedCostCenterById($id){
        try{

        $q= "SELECT cost_center_mapping.costcenter_id,  cost_center_mapping.location, cost_center_mapping.empId,hs_hr_employee.emp_firstname as empName, cost_center.name,cost_center_mapping.id,ohrm_location.name AS LocationName  from cost_center_mapping
        LEFT JOIN cost_center ON  cost_center.id = cost_center_mapping.costcenter_id
        LEFT JOIN hs_hr_employee on hs_hr_employee.emp_number = cost_center_mapping.empId
        LEFT JOIN ohrm_location ON ohrm_location.id = cost_center_mapping.location where cost_center_mapping.id = $id";
