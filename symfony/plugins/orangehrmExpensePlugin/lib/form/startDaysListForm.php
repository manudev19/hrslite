...
class startDaysListForm extends sfFormSymfony {
    ...
    public function configure() {
        ...
        $this->setWidgets(array(
            'startDates' => new sfWidgetFormSelect(array('choices' => $this->getStartAndEndDates($employeeId)),array('style'=>'width:auto;')),
        ));
    }
	...
	if($timesheets != null){
        ...
        foreach ($timesheets as $timesheet) {
            if($timesheet->getState()=='APPROVED'){
                $state='AP';
            } else if($timesheet->getState()=='SUBMITTED'){
                $state='NA';

            }else if($timesheet->getState()=='NOT SUBMITTED' || $timesheet->getState()=='REJECTED' ){
                $state='NS';  
                }
            ...
            if (isset($allowedActions[WorkflowStateMachine::TIMESHEET_ACTION_VIEW]) || $dataGroupPermissions->canRead()) {
                ...
                $dateOptions[$i] = $timesheet->getStartDate(). " ".__("to")." " . $timesheet->getEndDate().' - '.$state;
                $dateOptionsToDrpDwn[$i] = set_datepicker_date_format($timesheet->getStartDate() ). " ".__("to")." "  . set_datepicker_date_format($timesheet->getEndDate()).' - '.$state;
                $i++;
            }
        }
	}
