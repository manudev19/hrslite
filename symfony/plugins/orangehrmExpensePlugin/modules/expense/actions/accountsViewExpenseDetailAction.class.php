...
*/
class accountsViewExpenseDetailAction extends baseTimeAction {
  ...
  public function execute($request) {
    ...
    $request->setParameter('initialActionName', 'accountsExpenseReport');
    ...
    $this->data = array('empnum' => $result[1][0]['empNumber'],
    ...
    'financeStatus' => $result[1][0]['finance_status']
      );
      $index = 1;
      $demoObject = array();
        ...
        if($request->getParameter('btnComment')) {
          ...
          */
          ...
          foreach ($costCenterAdminDetails as $key => $singleSupervisor) {
            $emailData['recipientFirstName'] = $singleSupervisor['fullName'];
            $this->getEmailService()->sendEmailUsingTemplate('supCommentedExpenseToManReport', $emailData , $singleSupervisor['emailId']);
          }
...
//          }         
          ...
          if($request->getParameter('btnApprove')) {
            ...
            $this->getExpenseService()->accountTeamStatusUpdate($state,$this->expenseId, $finance_status); 
          /**
          *This is the details of expense report. It will be a comman body for for the employee and the manager
          */
          $expenseDetail = $this->getExpenseService()->getExpenseItem($this->expenseId);
          ...
          */
          ...
          $state= $x['state'];
          $date = $x['dateTime'];
          //$expenseUrlMan = 'expense/viewExpenseDetail?expenseId='.$request->getParameter('expenseId');
          //$expenseUrlMaAcc = 'expense/accountsExpenseReport?expenseId='.$request->getParameter('expenseId');
...
//            $this->getEmailService()->sendEmailUsingTemplate('supapprovedExpenseReport', $emailData , $singleSupervisor['emailId']);
//          }         
          $this->messageData = array('success', __('Successfully Processed'));
          $this->redirect($request->getReferer());
        ...
        if($request->getParameter('btnReject')){
          ...
          $this->getExpenseService()->accountTeamRejectedStatusUpdate($state,$this->expenseId);
          /**
          *This is the details of expense report. It will be a comman body for for the employee and the manager
          */
          $expenseDetail = $this->getExpenseService()->getExpenseItem($this->expenseId);
