...
//                $runtimeWhereClauseConditionArray = $reportGeneratorService->generateWhereClauseConditionArray($linkedFilterFieldIdsAndFormValues);
        ...
        $dataSet = self::escapeData($rawDataSet,$reportId);
    ...
    public function escapeData($data,$reportId) {
        if (is_array($data)) {
            $escapedArray = array();
            foreach ($data as $key => $rawData) {
                $escapedArray[$key] = self::escapeData($rawData,$reportId);
                 if( is_numeric($escapedArray[$key]) && $reportId=='4')
                 {
                 $escapedArray[$key]=$this->getTimesheetService()->convertDurationToHours($escapedArray[$key]);
                 $escapedArray[$key]=preg_replace('/\:/','.',   $escapedArray[$key]);
                 }
             }
            return $escapedArray;
        } else {
