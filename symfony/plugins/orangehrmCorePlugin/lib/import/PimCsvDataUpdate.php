 ...
 *
 */
class PimCsvDataUpdate extends CsvDataUpdate {
	...
	public function update($data) {
		...
		$newDate1 = date("Y-m-d", strtotime($data[6]));
		if ($this->isValidDate($newDate1)) {
			$employee->setEmpDriLiceExpDate($newDate1);
		}
		...
		$newDate = date("Y-m-d", strtotime($data[10])); 
		if ($this->isValidDate($newDate)) {
			$employee->setEmpBirthday($newDate);
		}
		...
		if (strlen($data[22]) <= 70) {
			$employee->setEmpAccountNum($data[22]);
		}
		if (strlen($data[23]) <= 70) {
			$employee->setEmpPanNum($data[23]);
		}
		if (strlen($data[24]) <= 50) {
			$employee->setEmpAadhaarNum($data[24]);
		}
        ...
        $empService -> updateEmployeesOnId($employee);
       // return true;
	}
	...
	private function isValidDate($date) {
		if (preg_match('/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/', $date)) {
			...
			header("Location: http://hrm.sti.com/symfony/web/index.php/admin/PimCsvUpdate?success=3", true, 301);
			exit();}
