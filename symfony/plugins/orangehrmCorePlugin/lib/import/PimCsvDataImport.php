...
class PimCsvDataImport extends CsvDataImport {
	...
	public function import($data) {
        ...
        if (strlen($data[22]) && strlen($data[22]) <= 50) {
			$employee->setEmpAadhaarNum($data[22]);
		}
		if (strlen($data[23]) && strlen($data[23]) <= 50) {
			$employee->setEmpPanNum($data[23]);
		}
		if (strlen($data[24]) && strlen($data[24]) <= 50) {
			$employee->setEmpAccountNum($data[24]);
		}
