...
class employeeInfoAction extends basePimAction{

    public function execute($request) {
        
        $this->form = new EmployeeInfoForm();


        if ($request->isMethod("post")) {


            $employee = $request->getParameter('employeeName');
            $employeeId =  $employee['empId'];
            $this->redirect('pim/viewEmployee?empNumber=' . $employeeId);
