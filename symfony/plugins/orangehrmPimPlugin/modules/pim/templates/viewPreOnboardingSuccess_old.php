...
?>
<style>#preonboarding .validation-error {
    color: red;
}
#issuing_date{
...
<label class="fieldDescription"><?php echo __('Issuing Date'); ?><em>*</em></label>
  <?php echo $form['issuing_date']->render(array("class" => "editable")); ?>
 ...
 foreach ($employeeValuesArray as $row) {?>
        <li>
            <?php echo $form['initialRows']['issuing_date']->renderLabel(__('Issuing Date')); ?>
  ...
  var currentDate = date.getFullYear() + "-" + (("0" + (date.getMonth() + 1)).slice(-2))
   ...
   if(joined_Date=='' )
   {
    $("#error_date").html("This field is required ");
        $('#error_date').show();
        error_date=true;
   }else if (!joined_Date.match(regEx) || joined_Date<currentDate) {
...
if(error_date==false && error_name==false &&error_manager==false && error_designation==false &&
error_department==false)
{
  return true;
}
...
<?php if (isset($_SESSION['isAdmin']) && $_SESSION['isAdmin'] != "Yes"&&(isset($_SESSION['isSupervisor']) && $_SESSION['isSupervisor'] == "Yes"||isset($_SESSION['empNumber']))) { ?>
 ...
 if(error_dedicated==false && error_international==false && error_location==false && error_workstation==false)
 {
  return true;
}
else
