...
class EmployeeDao extends BaseDao
{
    ...
    public function getSubordinateIdListBySupervisorId($supervisorId, $includeChain = false, $supervisorIdStack = array(), $maxDepth = NULL, $depth = 1)
    {
        ...
        try {
            ...
            if ($_SESSION['userRole'] == "Functional Head") {
                $q = "SELECT emp_number AS erep_sub_emp_number
                    FROM hs_hr_employee AS h INNER JOIN ohrm_subunit AS oh ON h.work_station = oh.id 
                        WHERE (oh.functional_head = ?)";
            } else {
     ...
     */
    public function getSubordinatePropertyListBySupervisorId(
        $supervisorId,
        $properties,
        $includeChain = false,
        $orderField = NULL,
        ...
        try {
            ...
            if ($_SESSION['userRole'] == "Functional Head") {
                $q->from('ReportTo rt')
                    ...
                    ->leftJoin('e.subDivision s ON e.work_station=s.id')
                    ->where("s.functional_head = ?", $supervisorId);
            } else {
                $q->from('ReportTo rt')
                    ->leftJoin('rt.subordinate e')
                    ->leftJoin('e.ReportTo rt2 ON rt.erep_sub_emp_number = rt2.erep_sup_emp_number')
            ...
            foreach ($subordinates as $subordinate) {
                ...
                if ($subordinate['isSupervisor'] && ($includeChain || (!is_null($maxDepth) && ($depth < $maxDepth)))) {
                    if (!in_array($subordinate['subordinateId'], $supervisorIdStack)) {
                        ...
                        $subordinatePropertyList = $this->getSubordinatePropertyListBySupervisorId(
        ...
        try {
            ...
            if ($_SESSION['userRole'] == "Functional Head") {
                $query = Doctrine_Query::create()
                    ...
                    ->leftJoin('emp.subDivision s ON emp.work_station=s.id')
                    ->where("s.functional_head = ?", $supervisorId)
                    ->andwhere("emp.emp_number !=?", $supervisorId);
            } else {
                $query = Doctrine_Query::create()
                    ->from('ReportTo rt')
                    ->leftJoin('rt.subordinate emp')
    ...
    public function getUnAssignedCurrencyList($empNumber, $salaryGrade, $asArray = false)
    {
        try {
            ...
            $q = Doctrine_Query::create()->select('c.currency_id, c.currency_name')
                ...
                ->andWhere(
                    'c.currency_id NOT IN (SELECT e.currency_id FROM EmployeeSalary e WHERE e.emp_number = ? AND e.sal_grd_code = ?)',
     ...
     * @return none
     */
    private function _getEmployeeListQuery(
        &$select,
        &$query,
        array &$bindParams,
        &$orderBy,
        ...
        */
        ...
        if (!empty($filters)) {
            ...
            foreach ($filters as $searchField => $searchBy) {
                if (
                    !empty($searchField) && !empty($searchBy)
    ...
    public function getDepartmentEmployees($workStation)
    {
        ...
        $query = "select emp_number as empNumber, emp_firstname as firstName, emp_middle_name as middleName, emp_lastname as lastName,hs_hr_employee.employee_id as employeeId, termination_id from hs_hr_employee where role_in_department IS NULL AND work_station = " . $workStation;
    ...
    {
        $query = "select emp_number as empNumber, emp_firstname as firstName, emp_middle_name as middleName, emp_lastname as lastName,employee_id as employeeId, termination_id from hs_hr_employee where termination_id is NULL";
    ...
    public function updateEmployeesOnId(Employee $employee)
    {
        try {
            $q = Doctrine_Query::create()->update('Employee')
                ->set(array('emp_firstname' => $employee['firstName']))

                ->where('employee_id=?', $employee['employeeId']);
            if ($employee['lastName'] != '') {
                $q->set('emp_lastName', '?', $employee['lastName']);
            }

            if ($employee['middleName'] != '') {
                $q->set('emp_middle_name', '?', $employee['middleName']);
            }
            if ($employee['nation_code'] != '') {
                $q->set('nation_code', '?', $employee['nation_code']);
            }
            if ($employee['emp_gender'] != '') {
                $q->set('emp_gender', '?', $employee['emp_gender']);
            }
            if ($employee['licenseNo'] != '') {
                $q->set('emp_dri_lice_num ', '?', $employee['licenseNo']);
            }
            if ($employee['otherId'] != '') {
                $q->set('emp_other_id', '?', $employee['otherId']);
            }
            if ($employee['emp_account_num'] != '') {
                if (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $employee['emp_account_num'])) {
                    header("Location: http://hrm.sti.com/symfony/web/index.php/admin/PimCsvUpdate?success=2", true, 301);
                    exit();
                } else {
                    $q->set('emp_account_num', '?', $employee['emp_account_num']);
                }
            }
            if ($employee['emp_dri_lice_exp_date'] != '') {
            ...
            if ($employee['emp_marital_status'] != '') {
                $q->set('emp_marital_status', '?', $employee['emp_marital_status']);
            }
            if ($employee['street2'] != '') {
                $q->set('emp_street2', '?', $employee['street2']);
            }
            if ($employee['street1'] != '') {
                $q->set('emp_street1 ', '?', $employee['street1']);
            }
            if ($employee['city'] != '') {
                $q->set('city_code', '?', $employee['city']);
            }
            if ($employee['country'] != '') {
                $q->set('coun_code', '?', $employee['country']);
            }
            if ($employee['province'] != '') {
                $q->set('provin_code', '?', $employee['province']);
            }
            if ($employee['emp_zipcode'] != '') {
                $q->set('emp_zipcode', '?', $employee['emp_zipcode']);
            }
            if ($employee['emp_hm_telephone'] != '') {
                $q->set('emp_hm_telephone ', '?', $employee['emp_hm_telephone']);
            }
            if ($employee['emp_mobile'] != '') {
                $q->set('emp_mobile', '?', $employee['emp_mobile']);
            }
            if ($employee['emp_work_telephone'] != '') {
                $q->set('emp_work_telephone', '?', $employee['emp_work_telephone']);
            }
            if ($employee['emp_work_email'] != '') {
                $q->set('emp_work_email', '?', $employee['emp_work_email']);
            }
            if ($employee['emp_oth_email'] != '') {
                $q->set('emp_oth_email', '?', $employee['emp_oth_email']);
            }
            if ($employee['emp_pan_num'] != '') {
                $q->set('emp_pan_num', '?', $employee['emp_pan_num']);
            }
            if ($employee['emp_aadhaar_num'] != '') {
                if (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $employee['emp_aadhaar_num'])) {
                    header("Location: http://hrm.sti.com/symfony/web/index.php/admin/PimCsvUpdate?success=2", true, 301);
                    exit();
                } else {
                    $q->set('emp_aadhaar_num', '?', $employee['emp_aadhaar_num']);
                }
            }
    ...
    public function saveFunctionalHead($empNumber, $functionalHead)
    {

        try {
            $q1 = Doctrine_Query::create()->update('SubUnit')
                ->set('functional_head', '?', 0)
                ->where('functional_head =?', $empNumber);
            $q1->execute();
            foreach ($functionalHead as $a) {

                $q = Doctrine_Query::create()->update('SubUnit')
                    ->set('functional_head', '?', $empNumber)
                    ->where('id=?', $a);

                $addFunctionalHead = $q->execute();
            }

            return  $addFunctionalHead;
    ...
    public function getFunctionalHeadEmployeeList($includeTerminated = false, $orderField = 'lastName', $orderBy = 'ASC')
    {

        $functionalhead_empNumber = ($_SESSION['empNumber']);
        ...
        try {
            ...
            $q = Doctrine_Query::create()
                ->from('Employee e')
                ->innerJoin('e.subDivision s ON e.work_station=s.id')
                ->where('s.functional_head=?', $functionalhead_empNumber)
                ->andwhere('e.emp_number !=?', $functionalhead_empNumber)
                ->orderBy("e.$orderField $orderBy");
    ...
    /** writing query for candidate number */
    public function getCandidateNumberForEmp($employeeId)
    {
        $q = "SELECT DISTINCT(candidate_number) as candidate_number
    FROM ohrm_preonboarding 
    ...
    public function getEmployeeDetail($candidate_number)
    {
        $q = "SELECT `candidate_number`, `issuing_date`, `joined_Date`, `firstname`, `middlename`, `lastname`, `designation`, `department`, `dedicated`, `international`, `location` as `locations`, `reporting_manager`, `workstation` FROM `ohrm_preonboarding` WHERE candidate_number=$candidate_number";
    ...
    public function getManagerDetail($candidate_number, $manager_id)
    {

        $q = "SELECT CONCAT(emp_firstname,' ',emp_middle_name,' ',emp_lastname)  as empName FROM hs_hr_employee Where emp_number='" . $manager_id . "'";
    ...
    public function getEmployeeDetailForTable($Login_employee)
    {
        $q = "SELECT pre.candidate_number AS candidate_no,
    ...
    hr_emp.emp_number as manager_employee_id,
    CONCAT(hr_emp.emp_firstname,' ',hr_emp.emp_lastname) AS reporting_manager
    FROM 
    ...
    WHERE 
    hr_emp.emp_number=pre.reporting_manager ORDER by pre.candidate_number";


        $q1 = "SELECT pre.candidate_number AS candidate_no,
    pre.joined_Date As joined_Date,
    CONCAT(pre.firstname,' ',pre.middlename,' ',pre.lastname) AS full_name,
    job.job_title AS designation,
    dept.name AS department,
    ...
    AND
    pre.reporting_manager='" . $Login_employee . "' ORDER by pre.candidate_number";
        ...
        $prepareQuery = $pdo1->prepare($q1);
        $prepareQuery->execute();
        $result1 = $prepareQuery->fetchAll(PDO::FETCH_ASSOC);
        return [$result, $result1];
    }

    public function savePreonboardEmployee($data)
    {

        $q = "INSERT INTO `ohrm_preonboarding`(`issuing_date`, `joined_Date`, `firstname`, `middlename`, `lastname`, `designation`, `department`,`reporting_manager`) VALUES ('" . $data['issuing_date'] . "','" . $data['joined_Date'] . "','" . $data['firstname'] . "','" . $data['middlename'] . "','" . $data['lastname'] . "','" . $data['designation'] . "','" . $data['department'] . "','" . $data['reporting_manager'] . "')";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $prepareQuery = $pdo->prepare($q);
        $prepareQuery->execute();
    }
    public function updatePreonboardEmployeeForManager($data)
    {

        $q = "UPDATE `ohrm_preonboarding` SET `issuing_date`='" . $data['issuing_date'] . "',
    `dedicated`='" . $data['dedicated'] . "',
    `international`='" . $data['international'] . "',
    `location`='" . $data['locations'] . "',
    `workstation`='" . $data['workstation'] . "'
     WHERE `candidate_number`='" . $data['candidate_number'] . "'";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $prepareQuery = $pdo->prepare($q);
        $prepareQuery->execute();
    }
    public function updatePreonboardEmployeeForAdmin($candidate_no, $data, $manager)
    {
        $q = "UPDATE `ohrm_preonboarding` SET `issuing_date`='" . $data['issuing_date'] . "',
`joined_Date`='" . $data['joined_Date'] . "',
    ...
    WHERE `candidate_number`='" . $candidate_no . "'";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $prepareQuery = $pdo->prepare($q);
        $prepareQuery->execute();
    }
    public function  selectPreonboardingManager($data)
    {
        $q1 = "SELECT DISTINCT(emp_number) from hs_hr_employee as hs_emp
		 WHERE emp_number = '" . $data['reporting_manager'] . "'";
    ...
    public function  selectPreonboardingUpdateManager($data)
    {
        $q1 = "SELECT DISTINCT(emp_number) from hs_hr_employee as hs_emp
		 WHERE emp_number = '" . $data . "'";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $prepareQuery = $pdo->prepare($q1);
        $prepareQuery->execute();
        $result = $prepareQuery->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function checkInvalidUpdateManagerName($data)
    {
        $q1 = "SELECT CONCAT(emp_firstname,' ',emp_middle_name,' ',emp_lastname) AS full_name FROM `hs_hr_employee`
			WHERE emp_number='" . $data . "'";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $prepareQuery = $pdo->prepare($q1);
        $prepareQuery->execute();
    ...
    public function checkInvalidManagerName($data)
    {
        $q1 = "SELECT CONCAT(emp_firstname,' ',emp_middle_name,' ',emp_lastname) AS full_name FROM `hs_hr_employee`
                WHERE emp_number='" . $data . "'";
        $pdo = Doctrine_Manager::connection()->getDbh();
    ...
    public function  checkEmployeeStatus($data)
    {
    ...
    public function  checkWorkstationNumber($data)
    {
        $q1 = "SELECT DISTINCT(workstation),candidate_number from `ohrm_preonboarding`
                    where workstation like '" . $data . "'";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $prepareQuery = $pdo->prepare($q1);
        $prepareQuery->execute();
        $result = $prepareQuery->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getEmployeeSubordinators($empNumber)
    {

        try {
            $query = Doctrine_Query::create()->select('Count(rt.erep_sub_emp_number)')
                ->from('ReportTo rt')
                ->innerJoin('rt.subordinate emp')
                ->where('rt.erep_sup_emp_number = ?', $empNumber);
            $count = $query->fetchOne(array(), Doctrine_Core::HYDRATE_SINGLE_SCALAR);
    ...
    public function  getEmpID($empnumber)
    {
        try {
            ...
            $query = "select employee_id as employeNumber  from hs_hr_employee where emp_number = " . $empnumber;
    ...
    public function  getEmpNum($empnumber)
    {
        try {
    ...
    public function  getEmpIDForLeave($empnumber)
    {
        try {
            $query = "select employee_id as employeNumber from hs_hr_employee where emp_number = " . $empnumber;
            $pdo = Doctrine_Manager::connection()->getDbh();
    ...
    public function getTimsheetWeeklyTotalHours($empnumber, $startdate, $endsate, $state)
    {
        if ($state != 'NOT SUBMITTED') {

            $query = "SELECT SUM(duration) as duration FROM ohrm_timesheet_item 
                      WHERE date>= '$startdate' and date<= '$endsate' and employee_id=$empnumber";
    ...
    public function getEmpByLocationIdBasedOnUserRole($locationId, $employee)
    {
        try {
            $q = Doctrine_Query::create()
                ->from('EmpLocations');
            if (is_array($locationId)) {
                $q->whereIn('locationId', $locationId);
            } else {
                $q->where('locationId = ?', $locationId)
                    ->andwhere('empNumber =?', $employee);
            }
            $employeelist =  $q->execute();
            foreach ($employeelist as $employee) {
    ...
    public function getEmpByLocationId($locationId)
    {
        try {
    ...
    public function getEmployeesBySubUnitAndLocationId($subUnits, $locationId, $includeTerminatedEmployees = false)
    {
        try {

            $q = Doctrine_Query::create()
                ->from('Employee e')
                ->leftJoin('e.EmpLocations el ON e.empNumber =el.empNumber')
                ->where('e.work_station =?', $subUnits)
                ->andwhere('el.locationId =?', $locationId);
    ...
    public function getEmployeeByDateAndTermination($frmdate, $todate, $jobTitle, $subUnit)
    {
        $query = " select DISTINCT(hr.emp_number) from hs_hr_employee as hs , ohrm_emp_termination as hr WHERE hr.termination_date>='" . $frmdate . "' AND hr.termination_date<='" . $todate . "' AND hr.emp_number =hs.emp_number AND hs.termination_id is NOT NULL";
        if ($subUnit != 0) {
            $query = $query . " AND hs.work_station='" . $subUnit . "'";
        }
        if ($jobTitle != 0) {
            $query = $query . " AND hs.job_title_code='" . $jobTitle . "'";
        }
        $query = $query . " ORDER BY emp_number";
        $pdo = Doctrine_Manager::connection()->getDbh();
        $prepareQuery = $pdo->prepare($query);
        $prepareQuery->execute();
        $result = $prepareQuery->fetchAll(PDO::FETCH_ASSOC);
        ...
        foreach ($result as $employee) {
            $employeeNumbers[] = $employee['emp_number'];
        }
    ...
    public function getEmployeeTerminatedEmployeeByLocation($employee, $location)
    {

        try {

            $q = Doctrine_Query::create()->select('empNumber')
                ->from('EmpLocations');
            if (is_array($employee)) {
                $q->whereIn('empNumber', $employee);
            }
            $q->andwhere('locationId =?', $location)
                ->orderBy('empNumber');

            $result = $q->execute();
            foreach ($result as $employee) {
                $employeeNumbers[] = $employee['empNumber'];
            }
