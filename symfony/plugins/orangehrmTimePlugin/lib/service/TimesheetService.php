...
class TimesheetService {
    ...
    public function getTimesheetListByEmployeeIdAndState($employeeIdList, $stateList, $limit,$offset) {
        return $this->getTimesheetDao()->getTimesheetListByEmployeeIdAndState($employeeIdList, $stateList, $limit,$offset);
    }
    ...
    public function createTimesheets($startDate, $employeeId) {
        ...
        $endDate = end($datesInTheCurrenTimesheetPeriod);
        $timesheet = $this->getTimesheetByStartDateAndEmployeeId($timesheetStartingDate, $employeeId);
		/*Change for cR-34 Start*/
		$employeeService = new EmployeeService();
        $employeeJoinedDate = $employeeService->getEmployee($employeeId);
		/*Change for cR-34 End*/
        if ($timesheet == null) {
            if ($this->checkForOverlappingTimesheets($timesheetStartingDate, $endDate, $employeeId) == 0) {
                $statusValuesArray['state'] = 1;
            }else if($employeeJoinedDate['joined_date'] > $startDate){
                   $statusValuesArray['state'] = 4;
            } else {
                $accessFlowStateMachineService = new AccessFlowStateMachineService();
                $tempNextState = $accessFlowStateMachineService->getNextState(WorkflowStateMachine::FLOW_TIME_TIMESHEET, Timesheet::STATE_INITIAL, "SYSTEM", WorkflowStateMachine::TIMESHEET_ACTION_CREATE);
    ...
    */
    ...
    public function getdepartmentonemployeeid($employeeid){
        return $this->getTimesheetDao()->getdepartmentonemployeeid($employeeid);

    }
    public function getemployeeid($empid){
        return $this->getTimesheetDao()->getemployeeid($empid);
    ...
    {
        return $this->getTimesheetDao()->getJoinedDate($empId);
    }
    /**
     * Getting termination Date for Gray out
     */
    public function getEmployeeTerminationDate($empid){
        //   var_dump($this->getTimesheetDao()->getEmployeeTerminationDate($empid));
           return $this->getTimesheetDao()->getEmployeeTerminationDate($empid);
       }
 /**
    ...
    public function getEmployeeTerminationId($empid){
        //   var_dump($this->getTimesheetDao()->getEmployeeTerminationDate($empid));
           return $this->getTimesheetDao()->getEmployeeTerminationId($empid);
       }
}
