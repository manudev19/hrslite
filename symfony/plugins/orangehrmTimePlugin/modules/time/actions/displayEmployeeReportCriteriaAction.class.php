...
class displayEmployeeReportCriteriaAction extends displayReportCriteriaAction {
    ...
    public function setReportCriteriaInfoInRequest($formValues) {
        ...
        $empName= str_replace('US', '', $empName);
        $empName= str_replace('CO', '', $empName);
        $pattern = '/^[0-9\.\s\-]+/';
        $empName=preg_replace($pattern, '', $empName);
      
        $employee['empName']= $empName;
        $this->getRequest()->setParameter('empName', $employee);
        $this->getRequest()->setParameter('empId', $empId);
    }
