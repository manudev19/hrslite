        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->getParameter('btnSave')) {
                ...
                if( $this->timesheetForm->getCSRFtoken() == $request->getParameter('_csrf_token')){
                                      ...
                                      $this->convertDurationToSeconds($timesheetItemDuration);
                                       ...
                                       if( $this->convertDurationToSeconds($timesheetItemDuration)>=86400)
                                       {
                                        $this->getUser()->setFlash('edittimesheet.warning', __('Should Be Less Than 24 and in HH:MM'));
                                         $this->redirect( 'time/editTimesheet?'. http_build_query(array('timesheetId'=> $this->timesheetId, 'employeeId'=> $this->employeeId , 'actionName' =>$backAction,'timeSheetAction'=>  $this->timesheetActon)));   
                    ...
                    $this->redirect('time/' . $backAction . '?' . http_build_query(array('message' => $this->messageData, 'timesheetStartDate' => $startingDate, 'employeeId' => $this->employeeId,'timeSheetAction'=>  $this->timesheetActon)));
                 }
