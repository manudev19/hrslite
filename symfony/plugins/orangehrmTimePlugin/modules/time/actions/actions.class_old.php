...
class timeActions extends sfActions {
    ...
    public function executeAddRow($request) {
        ...
        $joinedDate = $timesheetservice->getJoinedDate($employeeId);
        $employeeJoinedDate = $joinedDate['joined_date'];
        $ter_date = $timesheetservice->getEmployeeTerminationDate($employeeId);
        $currentWeeks = $this->_getWeeks($startDate, $days);
        $form = new TimesheetForm(array(), $values);
        $form->addRow($request->getParameter("num"), $values);
        return $this->renderPartial('addRow', array('form' => $form, 'num' => $request->getParameter("num"), 'noOfDays' => $noOfDays, 'employeeJoinedDate' => $employeeJoinedDate, 'ter_date' => $ter_date, 'currentWeeks' => $currentWeeks));
   
    }

    public function _getWeeks($start, $noOfDays)
    {
        $week = array($start);
        for ($i = 1; $i < $noOfDays; $i++) {
            $week[] = date('Y-m-d', strtotime($start . " +" . $i . " day"));
        }
        return $week;
    }
...
//
    ...
    public function executeViewPendingApprovelTimesheet(sfWebRequest $request) {
        ...
        $this->action= $request->getParameter("timeSheetAction");

        $this->getContext()->getUser()->setFlash('timesheetId', $timesheetId);
        $this->getContext()->getUser()->setFlash('TimesheetStartDate', $startDate);
    
        $this->redirect('time/viewTimesheet?' . http_build_query(array('employeeId' => $employeeId,"timeSheetAction"=> $this->action)));
    }
...
//                $msg = __("Timesheet already exists");
			...
			case $statusArray['state'] == 4:
				  $msg = array("4", $statusArray['joining_Date']);
//                $msg = __("Timesheet already exists");
				  return $this->renderText(json_encode($msg));
			break;
        }
