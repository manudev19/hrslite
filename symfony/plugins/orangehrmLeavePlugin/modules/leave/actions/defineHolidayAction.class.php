        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->holidayPermissions->canCreate() || $this->holidayPermissions->canUpdate()) {
                ...
                if ($this->form->isValid()) {
                    ...
                    $holidayObjectDate = $this->getHolidayService()->readHolidayByDate($date , null,  $subUnit);
                    ...
                    if ($this->editMode) {
                        ...
                        if ($date == $holidayObjectDate->getDate() || $holidayObjectDate->getRecurring() == 1) {
                            if( $subUnit == $holidayObjectDate->getSubUnit() ){
                                $allowToAdd = false;
                            }
                        }
                    }
                    ...
                    if (!$allowToAdd && !is_null($date)) {
                        ...
                        $this->getUser()->setFlash('error.nofade', __('Failed to Save: Sub Unit Already Exists'));
                    } else {
                        ...
                        $holidayObject->setSubUnit($post['sub_unit']);
                        ...
                        $this->getAuditLogService()->auditLogQuery('LEAVE','defineHoliday','Holidays');
