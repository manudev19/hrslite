...
class viewLeaveListAction extends baseLeaveAction {
    ...
    public function execute($request) {
        ...
        $LeaveTypeForReport = $this->_getFilterValue($values, 'leaveTypeForReport', null);
        ...
        $searchParams = new ParameterObject(array(
                    ...
                    'leaveTypeForReport'=>  $LeaveTypeForReport
                ));
        ...
        //$this->getAuditLogService()->auditLogQuery('LEAVE','viewLeaveList','Leave List');
    ...
    protected function downloadReport($searchParams) {
        ...
        $content = "Employee ID,Employee Name,Date,Leave Type,Leave Balance(Days),Number of Days,Status,Comments\n";
        ...
        foreach ($list as $leaveRequest) {
            ...
            $comment = $this->getLeaveRequestService()->getCommentsForLeave($leaveRequest->getId());
            $comments='';
                foreach($comment as $res){
                $comments.= $res['leavecomment']." | ";
            }
            $comments=  rtrim($comments, " | ");
            $empId= $this->getEmployeeService()->getEmpIDForLeave($leaveRequest->getEmpNumber());
            $content .= "$empId ,$empName,$leaveDateRange,  $leaveTypeName, $leaveBalance, $numOfDays, $leaveStatus, $comments\n";
        }
