...
class HolidayService extends BaseService {
     ...
     * @return Holiday $holiday
     */
    public function readHolidayByDate($date, OperationalCountry $operationalCountry = null , $subunit = null ) {

       $holiday = $this->getHolidayDao()->readHolidayByDate($date, $subunit,  $operationalCountry);
     ...
     * @return Holidays
     */
    public function searchHolidays($startDate = null, $endDate = null, $subunit = null,$limit,$offset) {

        $holidayList = $this->getHolidayDao()->searchHolidays($startDate, $endDate, $subunit,$limit,$offset);
        ...
        $results = array();
        $resultsTotal = array();
        foreach ($holidayList[1] as $holiday) {
        ...
        foreach ($holidayList[0] as $holiday) {
            if ($holiday->getRecurring() == 1) {
                ...
                for ($year = $startYear; $year <= $endYear; $year++) {
                    ...
                    if ($recurringDate >= $startDate && $recurringDate <= $endDate) {
                        ...
                        $resultsTotal[] = $recurringHoliday;
                    }
                }
                             
            } else {
                $resultsTotal[] = $holiday;
            }
        }
        
        usort($resultsTotal, function($a, $b) {
            return strtotime($a->getDate()) - strtotime($b->getDate());
        });

        return [array_values($resultsTotal),array_values($results)];
    }
     ...
     * 
     */
    public function isHoliday($day , $subunit = null ) {
        $holiday = $this->getHolidayDao()->readHolidayByDate($day , $subunit );
     ...
     * @return Holidays $holidayList
     */
    public function getFullHolidayList($subunit = null) {

        $holidayList = $this->getHolidayDao()->getFullHolidayList($subunit);
        return $holidayList;
    }
