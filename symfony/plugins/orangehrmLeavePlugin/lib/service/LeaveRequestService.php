...
class LeaveRequestService extends BaseService {
    ...
    public function getLeaveRequestActions($request, $loggedInEmpNumber) {
        ...
        if (!$request->isStatusDiffer()) {
			...
			if ($empNumber == $loggedInEmpNumber && (strtotime(date('Y-m-d')) > strtotime($request->date_applied))) {
                $actions = array();
            }
        }
    ...
    public function getLeaveActions($leave, $loggedInEmpNumber) {
        ...
        if ($leave->getEmpNumber() == $loggedInEmpNumber && (strtotime(date('Y-m-d')) > strtotime($leave->date))) {
                $actions = array();
        }
        return $actions;
    }    
    ...
    public function getCommentsForLeave($leaveId){
        return $this->getLeaveRequestDao()->getCommentsForLeave($leaveId);
        }
}
