...
class LeaveRequestDao extends BaseDao {
    ...
    public function searchLeaveRequests($searchParameters, $page = 1, $isCSVPDFExport = false, $isMyLeaveList = false,
        ...
        $leaveTypeId = $searchParameters->getParameter('leaveTypeForReport');
...
//        }
        ...
        if (!empty($employeeName)) {
            ...
            $employeeName= str_replace('US', '', $employeeName);
            $employeeName= str_replace('CO', '',  $employeeName);
                $employeeName= trim(preg_replace("/\s*(?:[\d_]|[^\w\s])+/", "", $employeeName));
            // Replace multiple spaces in string with wildcards
            $employeeName = preg_replace('!\s+!', '%', $employeeName);

            // Removed Surround with wildcard character to select only perticular employee details
...
//        }
    ...
    public function getCommentsForLeave($leaveId) {
 
        try {
            $query = "select comments as leavecomment from ohrm_leave_request_comment where leave_request_id = ".$leaveId;
