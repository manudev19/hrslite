...
class LeaveEntitlementDao extends BaseDao {
            ...
            if (count($newEmployeeList) > 0) {
                ...
                foreach ($newEmployeeList as $empNumber) {
                    ...
                    if (!isset($allEntitlements[$empNumber])) {
                        ...
                        if( $leaveEntitlement->getEntitlementType() == 2 ) {
                            ...
                            $noOfDays = $leaveEntitlement->getNoOfDays() * $leave_accural_mul;
    ...
    protected function getEntitlementType ($conn , $empNumber, $leaveTypeId){

        $sql  = 'SELECT entitlement_type from  ohrm_leave_entitlement where emp_number = ? and leave_type_id = ?  ';
        $parameters = array( $empNumber, $leaveTypeId );
    ...
    public function getLeaveBalance($empNumber, $leaveTypeId, $asAtDate, $date = NULL) {
        ...
        $leavetypeSql = " SELECT is_entitlement_not_required FROM ohrm_leave_type " .
                " WHERE id= ? ";


        $leavetypeParams = array($leaveTypeId);
        $statementLeavetype = $conn->prepare($leavetypeSql);
        $resultLeavetype = $statementLeavetype->execute($leavetypeParams);

        if( $resultLeavetype ){

           $resultLeavetype = $statementLeavetype->fetch(PDO::FETCH_ASSOC); 
           if (!empty($resultLeavetype['is_entitlement_not_required'])) {
   ...
   public function getEmpNumberFromLeave($userId) {
        try {
            $q = Doctrine_Query :: create()->select('emp_number')
