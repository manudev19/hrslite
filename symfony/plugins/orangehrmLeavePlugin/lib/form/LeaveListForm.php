...
class LeaveListForm extends sfForm {
    ...
    public function configure() {
        ...
        $validators = array();
        $defaults = array();
        $LeaveTypeList = $this->getSearchableLeaveTypeList();
        ...
        if ($this->mode != self::MODE_MY_LEAVE_LIST) {
            ...
            $widgets['leaveTypeForReport'] = new sfWidgetFormSelect(array('choices' => $LeaveTypeList));
             
            $labels['leaveTypeForReport'] = 'Leave Type';
            $validators['leaveTypeForReport'] = new sfValidatorChoice(array('required' => false, 
            'choices' => array_keys($LeaveTypeList))); 

            $widgets['txtEmployee'] = new ohrmWidgetEmployeeNameAutoFill(
                    array('loadingMethod'=>'ajax',
                          'requiredPermissions' => $requiredPermissions));
            
            $labels['txtEmployee'] = __('Employee Name/Id');
    ...
    private function getSearchableLeaveTypeList() {
        $list = array();
        $list[] = __("All");
        $leaveTypes = $this->getLeaveTypeService()->getLeaveTypeList();
        
      
        
        foreach ($leaveTypes as $leaveType) {
           
                $list[$leaveType->getId()] = $leaveType->getName();
