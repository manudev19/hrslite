...
$(document).ready(function() {
	...
	/*
	* Changes for CR-34 Added this function  Start
	*/
	$("#addTimesheetBtn").click(function() {
        ...
        if(validateDate(startdate, datepickerDateFormat)){
            ...
            /* Removed If condition for future timesheets the section for CR-34 */
                ...
                $.getJSON(url, function(data) {
                    ...
                    if(data[0]==4){
                        displayMessages('warning', lang_oldTimesheet_cannot_create);
                    }
                    if(data[0]==2){
            ...
            if($(element).val()){
                if(!(/^[0-9]+\.?[0-9]?[0-9]?$/).test($(element).val())) {
                    ...
                    if(!(/^[0-9]+\.?[0-9]?[0-9]?$/).test(temp[0]) || !(/^[0-9]+\.?[0-9]?[0-9]?$/).test(temp[1])){
                        displayMessages('warning', lang_if_alpha);
...
function clicked(dropdown){
 ...
 location.href = linkForViewTimesheet+"?timesheetStartDateFromDropDown="+dates[0]+"&selectedIndex="+selectedIndex+"&employeeId="+employeeId+"&timeSheetAction="+timsheetAction;
    }
