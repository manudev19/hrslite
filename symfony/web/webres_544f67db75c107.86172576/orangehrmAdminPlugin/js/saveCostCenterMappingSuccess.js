...
$(document).ready(function() {
    ...
    $('#btnCancel').click(function() {
        window.location.replace(cancelBtnUrl+'?id='+id);
    });
...
function setProjectAdmins(){
    ...
    var j = 0;
    $('.formInputCostCenter').each(function(){
        element = $(this);
        if($(element).val() == ''){
    ...
    $('.formInputProjectAdmin').each(function(){
        ...
        if(inputName != "" && inputName != "type for hints..."){
            var i;
            for (i=0; i < empCount; i++) {
                ...
                //alert('befor adding validation');
                $(element).addClass('validation-error');
                displayMessages('warning', please_fill_field); 
                //alert('after adding validation');
                flag = false;
        }
    });
...
function displayMessages(messageType, message) {
    ...
    if (messageType != 'reset') {
        ...
        "<a class='messageCloseButton' href='#'>"+closeText+"</a>" + 
        "</div>";
