...
class SeparationManagerConfirmationMailer extends Configuration
{
...
private final function sendMail($managers_email,$emp_mail,$template)
	{
		...
		$this->CI->email->from($managers_email, 'Employee Separation Confirmation Reminder');
		
	    $this->CI->email->to(SeparationManagerConfirmationMailer::HR_GROUP_MAIL);
	...
	public function SeparationManagerConfirmationEmailTemplate($data)
	{
		$message_head = "Hi,<br />";
		...
		$counter = 1;
			...
			foreach($data->result() as $result)
			{
				...
				if($result->Manager_Status=="Approved")	
				{	
					
					$this->sendMail($result->emp_manager_id,$result->emp_work_email,$message);
				}
				else if($result->Manager_Status=="Rejected")
				{
